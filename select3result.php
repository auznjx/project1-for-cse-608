...
if ($result->num_rows > 0){
    ...
    echo '<th>title</th><th>star1</th><th>star2</th>';
    echo '</tr>';
    }
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        echo '<tr>';
        echo  '<td>'. $row["title"].'<td>'. $row["star1"].'</td>'.  '<td>'. $row["star2"].'</td>'.'</td>';
    }
    echo '</table>';
} else {
    echo '<br>';
...
To get some detail about the star <br/> <br/>

<meta http-equiv="content-type" content="text/html;charset=utf-8"/> 
<form action="select3d.php" method="post">
starname         : <input type="text" name="name"/><br/>
